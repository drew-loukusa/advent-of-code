...
def adjacent_occupied(seats, x, y):
    ...
    if y > 0:
        if x > 0 and is_occupied(seats, x - 1, y - 1):
            occupied_seats += 1
        if is_occupied(seats, x, y - 1):
            occupied_seats += 1
        if x < MAX_X and is_occupied(seats, x + 1, y - 1):
            occupied_seats += 1

    # Count seats left and right: (x-1, y), (x+1, y)
    if x > 0 and is_occupied(seats, x - 1, y):
        occupied_seats += 1
    if x < MAX_X and is_occupied(seats, x + 1, y):
        occupied_seats += 1
