...
class Computer:
    def __init__(
        self, tape, name=None, inp_stack=None, output_mode=None, input_mode=None
    ):
        self.name = name
        self.enabled = True
    ...
    def clone(self):
        ...
        clone = Computer(
            self.tape[:], inp_stack=self.inp_stack[:], output_mode=self.output_mode
        )
        clone.i = self.i
...
if True:
    ...
    while pc.enabled:
        ...
        if output not in "vVX" and (
            output in abc or output in abc.lower() or output in ":?"
        ):
            # input("Pause...")
            continue
