...
class HullPainter():
    ...
    def get_loc(self): return self.x, self.y

    def get_dir(self): return self.dirs[self.dir_p]
    ...
    def move(self): 
        ...
        if cur_dir == 'up':     self.y -= 1
        if cur_dir == 'down':   self.y += 1
        if cur_dir == 'right':  self.x += 1
        if cur_dir == 'left':   self.x -= 1
    ...
    def paint_hull(self, visualize=False, interval=0):
        ...
        while self.computer.enabled:
            ...
            if new_color == 0: self.painted_black += 1
            else:              self.painted_white += 1

            # For visulazation: 
            if visualize:
                sleep(interval)
                col,row = cur_loc; col, row = col + G_OFFSET, row + G_OFFSET
                self.move_cursor(row,col)
                if new_color == 1: print('▮')
                if new_color == 0: print('.')
