...
class Computer:
    ...
    def clone(self):
        ...
        clone = Computer(
            self.tape[:], inp_stack=self.inp_stack[:], output_mode="return"
        )
        clone.i = self.i
...
class Path:
    ...
    def __init__(self, loc, prev):
        ...
        self.branches = (
            []
        )  # List of path objects defining possible braching paths from THIS path node
...
class RepairBot:
    ...
    def move_bot(self, exploring=False):
        ...
        if status == 0:
            wall = (0, 0)
            if self.direction == 1:
                wall = (self.loc[0], self.loc[1] - 1)
            elif self.direction == 2:
                wall = (self.loc[0], self.loc[1] + 1)
            elif self.direction == 3:
                wall = (self.loc[0] - 1, self.loc[1])
        ...
        # If the move was succesful, track it here in RepairBot:
            ...
            if self.visualize and not exploring:
                print_bot(self.loc, self.old_loc, self.direction)
            ...
            if self.visualize:
                for x in range(42):
                    ...
                    if (x, y) in self.walls:
                        print_symbol((x, y), 1, self.walls[(x, y)])
    ...
    def check_dir(self, direc, last_direc):
        if direc == 3 and last_direc == 4:
            return False
        if direc == 4 and last_direc == 3:
            return False
        if direc == 1 and last_direc == 2:
            return False
        if direc == 2 and last_direc == 1:
            return False
        return True
    ...
    def auto_run(self, limit=None):
        ...
        # Step 1:
        if limit:
            while self.move_bot() and self.steps_taken <= limit:
                pass
        else:
            while self.move_bot():
                pass
        ...
        while self.computer.enabled and not self.found_oxygen:
            if limit and self.steps_taken > limit:
                break
            # Step 2:
            while True:
                self.turn_left()
    ...
    def auto_run_inverted(self):
        ...
        while no_wall := self.move_bot():
            if not self.backtracking and no_wall:
                mv_cmds += 1
            elif self.backtracking and no_wall:
                mv_cmds -= 1
                ...
                if self.move_bot(exploring=True):
                    if direction != orig_direction and direction != prev_direction:
                        ...
                        # no need to clone a bot to do it, and don't go backwards!
                            ...
                            direction != orig_direction and direction != prev_direction
                        ):
                            if self.loc not in self.visited:
...
def travel_path(node: Path, printed_locs, path_length, longest=None):
    ...
    if path_length > longest[0]:
        longest[0] = path_length
...
if True:
    ...
    move_cursor(50, 0)
    print("Longest path length is: ", length[0] - 1)
