    ...
    for i in range(1000): 
        ...
        for a, m1 in moons.items():
            for b, m2 in moons.items():
                ...
                if m1.x < m2.x: m1.dx += 1; m2.dx -= 1
                if m1.x > m2.x: m1.dx -= 1; m2.dx += 1

                if m1.y < m2.y: m1.dy += 1; m2.dy -= 1
                if m1.y > m2.y: m1.dy -= 1; m2.dy += 1

                if m1.z < m2.z: m1.dz += 1; m2.dz -= 1
                if m1.z > m2.z: m1.dz -= 1; m2.dz += 1
...
while True:      
    ...
    for m1, m2 in pairs:
        if m1.x < m2.x: m1.dx += 1; m2.dx -= 1
        elif m1.x > m2.x: m1.dx -= 1; m2.dx += 1

        if m1.y < m2.y: m1.dy += 1; m2.dy -= 1
        elif m1.y > m2.y: m1.dy -= 1; m2.dy += 1

        if m1.z < m2.z: m1.dz += 1; m2.dz -= 1
        elif m1.z > m2.z: m1.dz -= 1; m2.dz += 1
